...
uint8_t median_filter(uint8_t window[3][3])  {
    ...
    COMPARE_SWAP(e[0],e[1]); COMPARE_SWAP(e[3],e[4]); COMPARE_SWAP(e[6],e[7]);
    COMPARE_SWAP(e[0],e[3]); COMPARE_SWAP(e[3],e[6]); COMPARE_SWAP(e[0],e[3]);
    COMPARE_SWAP(e[1],e[4]); COMPARE_SWAP(e[4],e[7]); COMPARE_SWAP(e[1],e[4]);
    COMPARE_SWAP(e[2],e[5]); COMPARE_SWAP(e[5],e[8]); COMPARE_SWAP(e[2],e[5]);
    COMPARE_SWAP(e[1],e[3]); COMPARE_SWAP(e[5],e[7]);
    COMPARE_SWAP(e[2],e[6]); COMPARE_SWAP(e[4],e[6]); COMPARE_SWAP(e[2],e[4]);
    COMPARE_SWAP(e[3],e[5]); COMPARE_SWAP(e[2],e[3]); COMPARE_SWAP(e[5],e[6]);


    uint8_t median = e[4]; // El valor central
    return median;
}
